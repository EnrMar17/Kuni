-- Perfil del paciente (spec sin complicaciones): fase de tratamiento por
-- variable (diabetes/hipertension), fecha de diagnostico por diagnostico, y
-- permite un segundo plan inicial de glucosa (ayuno + postprandial).
-- No agrega parametros posicionales nuevos a private.save_patient — todo
-- viaja dentro de p_input (mismo patron que el resto de sus campos), asi que
-- `create or replace function` basta; no hace falta el DROP de 0009 (eso
-- solo era necesario porque 0009 agregaba PARAMETROS nuevos a la firma).
--
-- NO SE APLICA EN ESTA SESION. Correr manualmente cuando el usuario lo
-- decida (`supabase db push` o el flujo que use este proyecto).
begin;

alter table public.patients
  add column diabetes_treatment_phase text
    check (diabetes_treatment_phase in ('estable_oral','ajuste_insulina','insulina_estable_hba1c')),
  add column hypertension_treatment_phase text
    check (hypertension_treatment_phase in ('controlada','en_ajuste'));

create or replace function private.save_patient(p_patient uuid, p_room uuid, p_doctor uuid,
...
declare v_unit uuid; v_zone text; v_patient public.patients; v_consent uuid; v_codes text[];
  ...
  v_plan jsonb; v_kind text; v_diag_row record;
begin
  ...
  if p_patient is null or p_input is null or jsonb_typeof(p_input) <> 'object'
    or p_input - array['fullName','birthDate','sex','clinicalRecord','curp','whatsappE164','bloodType',
      'initialRisk','initialRiskReason','diagnoses','consent','diabetesTreatmentPhase',
      'hypertensionTreatmentPhase','diagnosedOn'] <> '{}'::jsonb then
  ...
  if not coalesce(jsonb_typeof(p_input->'fullName')='string' and length(btrim(p_input->>'fullName')) between 3 and 300
    ...
    and jsonb_typeof(p_input->'diagnoses')='array' and p_input ? 'consent'
    and (p_input->>'diabetesTreatmentPhase' is null
      or p_input->>'diabetesTreatmentPhase' in ('estable_oral','ajuste_insulina','insulina_estable_hba1c'))
    and (p_input->>'hypertensionTreatmentPhase' is null
      or p_input->>'hypertensionTreatmentPhase' in ('controlada','en_ajuste'))
    and (not p_input ? 'diagnosedOn' or jsonb_typeof(p_input->'diagnosedOn')='object'),false) then
    raise exception using errcode='PT422',message='VALIDATION';
  end if;
  v_birth := (p_input->>'birthDate')::date;
  if v_birth > v_today then raise exception using errcode='PT422',message='VALIDATION'; end if;
  ...
  -- Fase de tratamiento solo tiene sentido si el diagnostico correspondiente
  -- sigue vigente en este mismo guardado.
  if p_input->>'diabetesTreatmentPhase' is not null and not (v_codes && array[
    'diabetes_type_1','diabetes_type_2','diabetes_gestational','diabetes_other']) then
    raise exception using errcode='PT422',message='VALIDATION',detail='Fase de tratamiento de diabetes requiere un diagnostico de diabetes.';
  end if;
  if p_input->>'hypertensionTreatmentPhase' is not null and not ('hypertension' = any(v_codes)) then
    raise exception using errcode='PT422',message='VALIDATION',detail='Fase de tratamiento de hipertension requiere diagnostico de hipertension.';
  end if;
  -- Fecha de diagnostico: mapa {condition_code: 'YYYY-MM-DD'}, solo para
  -- codigos que efectivamente se estan guardando en este llamado, sin fechas
  -- futuras.
  if p_input ? 'diagnosedOn' and p_input->'diagnosedOn' is not null then
    for v_diag_row in select key,value from jsonb_each(p_input->'diagnosedOn') loop
      if not (v_diag_row.key = any(v_codes))
          or jsonb_typeof(v_diag_row.value) is distinct from 'string'
          or v_diag_row.value#>>'{}' !~ '^\d{4}-\d{2}-\d{2}$'
          or (v_diag_row.value#>>'{}')::date > v_today then
        raise exception using errcode='PT422',message='VALIDATION',detail='Fecha de diagnostico invalida.';
      end if;
    end loop;
  end if;
  ...
  if p_create then
    insert into public.patients(id,unit_id,consulting_room_id,full_name,birth_date,sex,record_number,curp,
      ...
      diabetes_treatment_phase,hypertension_treatment_phase)
      values(p_patient,v_unit,p_room,btrim(p_input->>'fullName'),v_birth,p_input->>'sex',btrim(p_input->>'clinicalRecord'),
        ...
        p_input->>'diabetesTreatmentPhase',p_input->>'hypertensionTreatmentPhase');
  else
    update public.patients set full_name=btrim(p_input->>'fullName'),birth_date=v_birth,sex=p_input->>'sex',
      ...
      diabetes_treatment_phase=p_input->>'diabetesTreatmentPhase',hypertension_treatment_phase=p_input->>'hypertensionTreatmentPhase'
  ...
  insert into public.patient_diagnoses(unit_id,patient_id,condition_code,attributed_doctor_id,diagnosed_on)
    select v_unit,p_patient,c,p_doctor,(p_input->'diagnosedOn'->>c)::date from unnest(v_codes) c where not exists(
  ...
  -- Permite corregir/agregar la fecha de un diagnostico ya activo sin
  -- reabrirlo (no se toca `active`/`attributed_doctor_id` de una fila que ya
  -- estaba vigente, solo su fecha, y solo si el payload la trae).
  update public.patient_diagnoses set diagnosed_on=(p_input->'diagnosedOn'->>condition_code)::date
    where unit_id=v_unit and patient_id=p_patient and active and condition_code=any(v_codes)
      and p_input->'diagnosedOn' ? condition_code;
  ...
  -- Planes de monitoreo iniciales: como mucho uno de presion, y hasta DOS de
  -- glucosa (ayuno + postprandial) siempre que traigan measurementContext
  -- distinto entre si — vigencia desde hoy. Los umbrales que no vengan
  -- quedan NULL a proposito (0001: "NULL en un limite significa no
  -- configurado. No clasificar como normal por ausencia de rango"); las
  -- restricciones CHECK de monitoring_plans son la ultima palabra sobre
  -- coherencia de rangos.
  if p_plans is not null then
    if jsonb_typeof(p_plans) is distinct from 'array' or jsonb_array_length(p_plans) > 3 then
      raise exception using errcode='PT422',message='VALIDATION',detail='Como mucho un plan de presion y dos de glucosa.';
    end if;
    if (select count(*) from jsonb_array_elements(p_plans) p where p->>'kind'='blood_pressure') > 1
        or (select count(*) from jsonb_array_elements(p_plans) p where p->>'kind'='glucose') > 2 then
      raise exception using errcode='PT422',message='VALIDATION',detail='Como mucho un plan de presion y dos de glucosa.';
    end if;
    if (select count(distinct coalesce(p->>'measurementContext','')) from jsonb_array_elements(p_plans) p where p->>'kind'='glucose')
        <> (select count(*) from jsonb_array_elements(p_plans) p where p->>'kind'='glucose') then
      raise exception using errcode='PT422',message='VALIDATION',detail='Los dos planes de glucosa deben tener un contexto distinto (ayuno vs. postprandial).';
    end if;
