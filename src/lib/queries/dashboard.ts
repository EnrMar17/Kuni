...
export async function loadDashboardRows(client: SupabaseClient<Database>, scope: { unitId: string; roomId: string }, now: Date): Promise<{ rows: DashboardRows; hasMorePatients: boolean }> {
  ...
  const emptyRows: DashboardRows = { patients, diagnoses: [], plans: [], measurements: [], interactions: [], responses: [], prescriptions: [], appointments: [], alerts: [], complications: [], nonresponse: [], consent: [] };
...
async function loadRelatedRows(client: SupabaseClient<Database>, scope: { unitId: string; roomId: string }, ids: string[], now: Date): Promise<Omit<DashboardRows, "patients">> {
  ...
  const [diagnoses, plans, measurements, interactions, responses, prescriptions, appointments, alerts, complications, nonresponse, consent] = await Promise.all([
    ...
    readAllDashboardRows((from, to) => client.from("prescriptions").select("*, medications(name, strength, therapeutic_class), prescription_schedules(local_time, weekdays)", { count: "exact" }).eq("unit_id", scope.unitId).in("patient_id", ids).eq("status", "active").order("id").range(from, to)),
  ...
  return { diagnoses, plans, measurements, interactions, responses, prescriptions, appointments, alerts, complications, nonresponse, consent };
}
