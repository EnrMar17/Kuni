...
export function PredictionRefreshButton({ onRefresh, refreshing }: { onRefresh?: () => void; refreshing?: boolean }) {
  ...
  return (
    ...
    >
      <svg aria-hidden="true" className={`size-4 ${busy ? "animate-spin" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
        <path strokeLinecap="round" strokeLinejoin="round" d="M20 11a8 8 0 1 0-2.34 5.66M20 4v7h-7" />
      </svg>
      {busy ? "Actualizando…" : "Actualizar predicción"}
    </button>
  );
}
